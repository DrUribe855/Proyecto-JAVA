...
public class DataBase {
    ...
    public ListaFacturas[] listaFacturas() {
        ...
        try {
            ...
            if (registros.getRow() == 1) {
                int indice = 0;
                do {
                    ListaFacturas temp = new ListaFacturas(registros.getInt("cedula_cliente"), registros.getInt("cedula_vendedor"), registros.getInt("id"), registros.getString("fecha"), registros.getInt("total"),registros.getInt("id"));
    ...
    public ResultSet consultarItemsFactura(int id) {
        try {
           ResultSet itemsFactura = this.manipularDB.executeQuery("SELECT productos.*, items_factura.cantidad, items_factura.subtotal FROM items_factura INNER JOIN productos ON items_factura.id_producto = productos.id WHERE items_factura.id_factura = " + id);
